...
///Delete old records
    //$fs->delete_area_files($context->id, 'mod_journal', 'private', $fid);
      
    $file_record = new stdClass;
    
    if (!empty($id)) {
      $file_record->component = 'mod_journal';
    ...
    if(isset($itemid) && is_numeric($itemid)) {
      ...
      if ($files = $fs->get_area_files($context->id, 'mod_journal', 'private', $fid)){
        $s = 2;
        $fid = rand(1, 999999999);
        while ($files = $fs->get_area_files($context->id, 'mod_journal', 'private', $fid)) {
          $s = 3;
          $fid = rand(1, 999999999);
        }
      }
    ...
    echo json_encode(array("id"=>$fid, "url"=>"/pluginfile.php/".$item->contextid."/mod_journal/".$id."/".$item->id."/".$item->filename));

...
function journal_print_user_entry($course, $user, $entry, $teachers, $grades) {
    ...
    if ($entry) {
        ...
        echo "<br />";
        echo html_writer::link('#', get_string("voiceannotations", "journal"), array("onclick"=>"document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block';getSel();$('#iid_1').val({$entry->id});startRecording(1);$('#idoftext_1').val($(this).parent().attr('id'));return false;"));  //FIX DISI
    } else {
        print_string("noentry", "journal");
    }
    echo "</td></tr>";
...
function journal_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload) {
    global $CFG, $DB;
    ...
    if ($file_record = $DB->get_record('files', array('id'=>$id))) {
      ...
      journal_send_stored_file($file, 86400, 0, false);  //forcedownload false
...
function journal_send_stored_file($stored_file, $lifetime=86400 , $filter=0, $forcedownload=false, $filename=null, $dontdie=false) {
    ...
    if (empty($filter)) {
        ...
        journal_prepare_file_content_sending();
...
function journal_prepare_file_content_sending() {
